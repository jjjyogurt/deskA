...
// status: button works; mic works; audio can be transfered;but voice agent stuck at thinging 
...
// Desk control button (UNCHANGED behavior)
...
// Audio capture settings (best match for Android Volcengine ASR path)
...
constexpr size_t MIC_MAX_PCM_BYTES = MIC_SAMPLE_RATE * MIC_CHANNELS * (MIC_BITS_PER_SAMPLE / 8) * 60; // 1,920,000

// BLE packet format:
// [0..1] seq (uint16 LE), [2] flags (bit0=last), [3] reserved, [4..] payload
constexpr size_t AUDIO_PACKET_TOTAL_BYTES = 20;   // safe for default MTU=23
constexpr size_t AUDIO_PACKET_HEADER_BYTES = 4;
constexpr size_t AUDIO_PACKET_PAYLOAD_BYTES = AUDIO_PACKET_TOTAL_BYTES - AUDIO_PACKET_HEADER_BYTES;
constexpr uint8_t AUDIO_FLAG_LAST = 0x01;
constexpr int AUDIO_PACKETS_PER_LOOP = 4;         // limit to keep loop responsive
...
uint32_t micNotifyCount = 0;

// I2S capture temp buffer (32-bit input from INMP441)
...
// Desk control logic kept unchanged.
void processButton() {
...
bool setupI2sInput() {
  ...
  // INMP441 can have DC offset; discard startup garbage
...
bool ensureMicBuffer() {
  ...
  // Prefer PSRAM for 1-minute audio
  if (psramFound()) {
...
void processMicButton() {
  ...
  if ((now - lastMicDebounceMs) > DEBOUNCE_MS && raw != stableMicButton) {
    ...
    // Trigger tap on release edge.
    if (stableMicButton == BUTTON_INACTIVE_LEVEL) {
      if (DEBUG_MIC) {
        Serial.printf("[MIC] Tap release at %lu ms\n", now);
      }
      handleMicTap();
    }
  }
...
inline void appendSample16ToBuffer(int16_t sample) {
  if (micBufferedBytes + 2 > MIC_MAX_PCM_BYTES) {
    // Will be handled by caller after loop.
    return;
  }
  // little-endian
  micPcmBuffer[micBufferedBytes++] = static_cast<uint8_t>(sample & 0xFF);
  micPcmBuffer[micBufferedBytes++] = static_cast<uint8_t>((sample >> 8) & 0xFF);
}
...
void captureMicIfRecording() {
  ...
  if (err != ESP_OK) {
    if (DEBUG_MIC) {
      Serial.printf("[MIC] i2s_read err=%d\n", err);
    }
    return;
  }
  ...
  for (size_t i = 0; i < samplesRead; i++) {
    ...
    // INMP441 delivers signed 24-bit data left-aligned in 32-bit frame.
    int32_t s32 = i2sReadBuffer[i];
    int16_t s16 = static_cast<int16_t>(s32 >> 14); // downscale to 16-bit PCM
    appendSample16ToBuffer(s16);
  }
}

void sendAudioPacket(const uint8_t *payload, uint16_t payloadLen, bool isLast) {
  if (audioChar == nullptr) {
    Serial.println("[MIC] audioChar unavailable");
    return;
  }

  uint8_t packet[AUDIO_PACKET_TOTAL_BYTES];
  packet[0] = static_cast<uint8_t>(micTransferSeq & 0xFF);
  packet[1] = static_cast<uint8_t>((micTransferSeq >> 8) & 0xFF);
  packet[2] = isLast ? AUDIO_FLAG_LAST : 0x00;
  packet[3] = 0x00;

  if (payloadLen > 0 && payload != nullptr) {
    memcpy(packet + AUDIO_PACKET_HEADER_BYTES, payload, payloadLen);
  }

  audioChar->setValue(packet, AUDIO_PACKET_HEADER_BYTES + payloadLen);
  audioChar->notify();
...
void sendMicTransferStep() {
  ...
  while (packetsSentThisLoop < AUDIO_PACKETS_PER_LOOP && micTransferPending) {
    if (micTransferZeroOnly) {
      sendAudioPacket(nullptr, 0, true);
    ...
    sendAudioPacket(payloadPtr, payloadLen, isLast);
...
void setupBlePeripheral() {
  ...
  // New audio notify characteristic
  ...
  Serial.print("[BLE] Audio characteristic: ");
  Serial.println(AUDIO_CHARACTERISTIC_UUID.toString().c_str());
}
...
void setup() {
  ...
  Serial.printf("[CFG] MIC_BUTTON_PIN=%d (toggle)\n", MIC_BUTTON_PIN);
  ...
  Serial.printf("[CFG] Initial desk button raw=%u stable=%u\n", lastRawButton, stableButton);
  Serial.printf("[CFG] Initial mic button raw=%u stable=%u\n", lastRawMicButton, stableMicButton);
...
void loop() {
  // Keep desk control feature behavior unchanged.
  processButton();
  ...
  if (DEBUG_BUTTON || DEBUG_MIC) {
    ...
    if ((now - lastPinDebugMs) >= PIN_DEBUG_INTERVAL_MS) {
      lastPinDebugMs = now;
      Serial.printf("[PIN] deskRaw=%u deskStable=%u click=%u micRaw=%u micStable=%u micRec=%s micTx=%s connected=%s\n",
